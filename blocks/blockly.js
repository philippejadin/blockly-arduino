...
Blockly.Blocks["variables_set_type"] = {
    init: function() {
        ...
        this.appendDummyInput().appendField(Blockly.Msg.VARIABLES_AS).appendField(new Blockly.FieldDropdown(Blockly.Types.getValidTypeArray()), "VARIABLE_SETTYPE_TYPE");
        this.setInputsInline(true);
        this.setOutput(true);
        this.setTooltip(Blockly.Msg.type)
    },
    getBlockType: function() {
...
Blockly.Blocks["controls_flow_statements"] = { init: function() {
    ...
    onchange: function() {
        ...
        var block = this;
        do {
            if (block.type == "controls_repeat_ext" || block.type == "controls_for" || block.type == "controls_whileUntil") {
                legal = true;
                break
            }
...
Blockly.Blocks["array_create_with"] = {
    ...
    decompose: function(workspace) {
        var containerBlock = Blockly.Block.obtain(workspace, "array_create_with_container");
        containerBlock.initSvg();
        var connection = containerBlock.getInput("STACK").connection;
        for (var i = 0; i < this.itemCount_; i++) {
...
Blockly.Blocks["procedures_defnoreturn"] = {
    ...
    updateParams_: function() {
        ...
        if (badArg) this.setWarningText(Blockly.Msg.PROCEDURES_DEF_DUPLICATE_WARNING);
        else this.setWarningText(null);
        var paramString = "";
        if (this.arguments_.length) paramString = Blockly.Msg.PROCEDURES_BEFORE_PARAMS + this.arguments_.join(",")+")";
    ...
    mutationToDom: function(opt_paramIds) {
        var container = document.createElement("mutation");
        if (opt_paramIds) container.setAttribute("name", this.getFieldValue("NAME"));
        for (var i = 0; i < this.arguments_.length; i++) {
            var parameter = document.createElement("arg");
            parameter.setAttribute("name", this.arguments_[i]);
            if (opt_paramIds && this.paramIds_) parameter.setAttribute("paramId", this.paramIds_[i]);
    ...
    domToMutation: function(xmlElement) {
        ...
        for (var i = 0, childNode; childNode = xmlElement.childNodes[i]; i++)
            if (childNode.nodeName.toLowerCase() == "arg") this.arguments_.push(childNode.getAttribute("name"));
        this.updateParams_();
...
Blockly.Blocks["procedures_mutatorarg"] = {
    init: function() {
        ...
        this.appendDummyInput().appendField(field, "NAME").appendField(Blockly.Msg.PROCEDURES_MUTATORARG_TYPE).appendField(new Blockly.FieldDropdown(Blockly.Types.getValidTypeArray()), "VARIABLE_SETTYPE_TYPE");
        this.setPreviousStatement(true);
        this.setNextStatement(true);
        this.setColour(Blockly.Blocks.procedures.HUE);
    ...
    updateShape_: function() {
        for (var i = 0; i < this.arguments_.length; i++) {
            ...
            if (field) {
                ...
                var input = this.appendValueInput("ARG" + i).appendField(field, "ARGNAME" + i).appendField("=");
                input.init()
            }
        }
